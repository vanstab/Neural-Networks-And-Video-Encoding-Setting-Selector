...
void DoubleTuple::DoubleTupleInit(int depth, int* networkColsSize){
	...
	bias = new double*[networkColsSize[depth]];
	for (int d = 0; d < networkColsSize[depth]; d++){
			bias[d] = new double[1];
			bias[d][0] = 0;
		}

	weight = new double*[networkColsSize[depth]];
	for (int d = 0; d < networkColsSize[depth]; d++){
...
void DoubleTuple::clear(){
	for (int d = 0; d < colSize[depthOfNetwork]; d++){
		bias[d][0] = 0;
	}
	for (int d = 0; d < colSize[depthOfNetwork]; d++){
